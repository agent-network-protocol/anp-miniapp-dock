use demo_server::auth::{TokenRecord, TokenScope};
...
async fn spawn_server() -> (SocketAddr, DemoState) {
...
async fn business_apis_fail_closed_without_or_with_expired_token() {
    ...
    state.insert_token_for_test(TokenRecord {
        token: "expired-token".to_owned(),
        scope: TokenScope {
            merchant_did: state.merchant_did().to_owned(),
